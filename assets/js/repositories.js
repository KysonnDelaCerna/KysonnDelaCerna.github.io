const app = Vue.createApp({
    template: `
        <div class="relative bg-fixed bg-cover bg-gray-900" style="background-image: url(https://picsum.photos/1920/1080)">
            ...
            <div v-if="repositories" v-for="repository in repositories" class="pb-4 relative z-10">
                <div class="flex flex-row justify-between bg-white bg-opacity-90 w-11/12 lg:w-4/5 mx-auto rounded-lg p-5 filter drop-shadow-lg">
                    <div>
                        ...
                        <h2 class="font-semibold text-lg">{{ repository.description }}</h2>
                        <h2>Made with: {{ repository.language }}</h2>
                    </div>
                    <div class="pl-4">
                        <a :href="repository.html_url"><button class="rounded-md bg-indigo-500 text-white font-semibold p-2 hover:bg-indigo-700 mb-2 float-right">
                            Repository
                        </button></a>
                        <br/>
                        <a v-if="repository.homepage" :href="repository.homepage"><button class="rounded-md bg-indigo-500 text-white font-semibold p-2 hover:bg-indigo-700 float-right">
                            Website
                        </button></a>
                    </div>
                </div>
